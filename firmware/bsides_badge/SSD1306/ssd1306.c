...
void ssd1306_refresh(void) {
  ...
  ssd1306_cmd(SSD1306_OFFSET + SSD1306_W -
              1);  // Column end address (127 = reset)
...
void ssd1306_drawImage(const unsigned char* input,
  ...
  for (uint8_t line = 0; line < height; line++) {
    ...
    // SSD1306 is in vertical mode, yet we want to draw horizontally, which
    // necessitates assembling the output bytes from the input data bitmask for
    // current pixel in vertical (output) byte
    ...
    for (uint8_t byte = 0; byte < bytes_to_draw; byte++) {
      ...
      for (pixel = 0; pixel < 8; pixel++) {
        ...
        // looking at the horizontal display, we're drawing bytes bottom to top,
        // not left to right, hence y / 8
        ...
        switch (color_mode) {
          case COLOR_NORMAL:
            ...
            ssd1306_buffer[buffer_addr] =
                (ssd1306_buffer[buffer_addr] & ~v_mask) |
 ...
 *  draw a rectangle
 */
void ssd1306_drawRect(uint8_t x,
                      uint8_t y,
                      uint8_t w,
                      uint8_t h,
                      ssd1306_color_mode_t color) {
 ...
 * fill a rectangle
 */
void ssd1306_fillRect(uint8_t x,
                      uint8_t y,
                      uint8_t w,
                      uint8_t h,
                      ssd1306_color_mode_t color) {
...
void ssd1306_drawchar_sz(uint8_t chr,
  ...
  for (i = 0; i < 8; i++) {
    ...
    for (j = 0; j < 8; j++) {
      ...
      // Draw the pixel at the original size and scaled size using nested
      // for-loops
      for (uint8_t k = 0; k < font_scale; k++) {
        for (uint8_t l = 0; l < font_scale; l++) {
          ssd1306_drawPixel(x + (j * font_scale) + k, y + (i * font_scale) + l,
                            col);
        }
      }
