 ...
 *
 * This header file contains the interface for managing the flash memory on the
 * CH32V003 microcontroller. It provides functions for storing and retrieving
 * non-volatile settings in the flash memory.
 *
 * @note Integrated flash has limited write cycles (10K as per datasheet) and
 * will wear out quickly without proper care. For frequent writes, consider
 * using external flash chips for increased durability.
 *
 * @note CH32V003 has 16K of flash memory, which is divided into 256 pages of 64
 * bytes each.
 *
 * @mainpage Flash Memory Management for CH32V003
 *
 * This library allows for the storage and retrieval of non-volatile settings in
 * the CH32V003 microcontroller's flash memory. It provides functions to write,
 * read, and erase flash memory pages and manage option bytes.
 *
 * @section usage Usage
 * To store variables in the main flash, reserve one or more pages at the end of
 * the main flash for storing non-volatile variables. Follow the provided
 * guidelines in overrides.ld and your Makefile to set the flash length.
 *
 * During the boot phase, call flash_set_latency() once.
 *
 * Read operations (getter functions) can be performed at any time and do not
 * require unlocking.
 *
 ...
 * 3. Write to option bytes; this will internally erase them and restore other
 * option byte values.
 * 4. Lock the flash.
 *
 * @section technical_notes Technical Notes
 * - Trying to write to addresses outside the main flash will likely freeze the
 * microcontroller, so ensure your code doesn't attempt it.
 * - Option bytes store data as IIIIIIII DDDDDDDD, with D as data (byte0) and I
 * as the inverse of data (byte1).
 * - To write a byte (8 bits), the write needs to be 16 bits (uint16_t), with
 * the upper 16 bits (I) replaced with the inverted bit pattern of D.
 * - Use provided functions for reading and writing data1 and data0 bytes or
 * manipulate OB->Data1 yourself if you require raw speed.
 *
 * @section address_calculations Address Calculations
 * To calculate the address for storing variables in the main flash, use the
 * following formula: \f$ \text{address of byte nonvolatile}[n] =
 * \text{FLASH_BASE} + \text{N_BYTES} + [n] \f$ where n is the offset from the
 * start of your reserved nonvolatile storage.
 *
 * For example:
 ...
 *
 * @note It is suggested to calculate all non-volatile storage addresses at the
 * beginning of the main and store them in variables. Alternatively, use the
 * FLASH_PRECALCULATE_NONVOLATILE_ADDR(n) preprocessor macro to define your
 * addresses, allowing the math to be done at compile time.
 */
...
#include "ch32v00x.h"
// #include "../Peripheral/inc/ch32v00x_flash.h"
 ...
 * The address can also be calculated at compile time using
 * FLASH_PRECALCULATE_NONVOLATILE_ADDR(n).
 *
 * @param byte_number The offset byte number from the start of nonvolatile
 * storage.
 ...
 *
 * This function sets the flash controller latency in accordance with the
 * FUNCONF_SYSTEM_CORE_CLOCK speed. It should be called during the boot phase.
 */
static inline void flash_set_latency();
/**
 * @brief Unlock the main flash for altering.
 *
 * This function unlocks the main flash memory to allow modifications such as
 * erase or program operations.
 */
 ...
 * It should be used in conjunction with flash_unlock() when modifying option
 * bytes.
 */
 ...
 *
 * This function locks the flash memory after completing modifications to
 ...
 *
 * This function erases a 64-byte page in the flash memory. Note that smaller
 * erases are not possible on the CH32V003.
 *
 ...
 */
static inline void flash_program_2x8_bits(uint32_t addr,
                                          uint8_t byte1,
                                          uint8_t byte0);
/**
 * @brief Program a float value into flash memory.
 *
 * This function programs a float value into flash memory by breaking it down
 * into two 16-bit values.
 *
 ...
 *
 * This function reads 16 bits of data from the specified address in flash
 * memory.
 *
 ...
 *
 * This function reads an 8-bit value from the specified address in flash
 * memory.
 *
 * @param addr The address from which the data will be read.
 ...
 *
 * This function reads a float value from the specified address in flash memory
 ...
 *
 * This function writes 16 bits of data to the option bytes while keeping other
 * contents intact. It involves backing up, erasing, and then writing the new
 * data.
 *
 ...
 * It utilizes the flash_write_option_byte_16_bits function for the actual
 * writing.
 *
 * @param data1 The first 8-bit value to be written.
 * @param data0 The second 8-bit value to be written.
 */
static inline void flash_write_option_byte_2x8_bits(uint8_t data1,
                                                    uint8_t data0);
/**
 ...
 *
 * This function reads both DATA1 and DATA0 option bytes from the option bytes
 * area and combines them into a 16-bit value.
 *
 ...
 *
 * This function returns the operational status of the flash, indicating whether
 * it is busy with an ongoing operation.
 *
 ...
 *
 * This function checks the status of the last flash operation, specifically if
 ...
 *
 * This function checks if a write protection error has occurred during the last
 * flash operation.
 *
 * @return uint8_t Non-zero if a write protection error occurred, zero
 * otherwise.
 */
 ...
 *
 * This function clears the End Of Programming (EOP) bit in the flash status
 * register.
 */
 ...
 *
 * This function blocks execution until the flash memory is no longer busy with
 ...
 *
 * This function is used to test if the first 8 bits of the input are the
 * inverse of the last 8 bits, typically used for data validation.
 *
...
// Internal variables
extern char FLASH_LENGTH_OVERRIDE
...
#define FLASH_PRECALCULATE_NONVOLATILE_ADDR(n) \
  FLASH_BASE + (uint32_t) (uintptr_t) (FLASH_LENGTH_OVERRIDE) + n
// Function Definitions
static inline uint32_t flash_calculate_runtime_address(uint16_t byte_number) {
  // Calculate the non-volatile storage address by adding the base address of
  // the flash, the override length (to account for any offsets), and the
  // specific byte number requested.
  return (FLASH_BASE + (uint16_t) (uintptr_t) FLASH_LENGTH_OVERRIDE +
          byte_number);
}
static inline void flash_set_latency() {
...
static inline void flash_program_2x8_bits(uint32_t addr,
                                          uint8_t byte1,
                                          uint8_t byte0) {
  // Combines two 8-bit values into a 16-bit value and programs it into flash at
...
static inline void flash_OB_erase() {
  ...
  // Set the start bit in the flash control register to begin the erase
  // operation.
  FLASH->CTLR |= CR_STRT_Set;
  // Wait until the flash is not busy before proceeding.
  // This ensures that the erase operation is complete before any further
  // actions.
  flash_wait_until_not_busy();
